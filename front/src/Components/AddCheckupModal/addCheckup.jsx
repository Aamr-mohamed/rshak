...
const AddCheckupModal = ({ onClose, isModalOpen, userId, onChange }) => {
  ...
  const editSchema = yup.object().shape({
    checkupName: yup.string().required("الرجاء إدخال اسم التحليل"),
    checkupDate: yup.string().required("الرجاء إدخال تاريخ التحليل"),
    checkupPercentage: yup.string(),
    checkupAverage: yup.string(),
    checkupDescription: yup.string().required("الرجاء إدخال تفاصيل التحليل"),
  ...
  const verification = async (values) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${backendUrl}/user/checkup/add/${userId}`,
        formData,
        {
          headers: {
