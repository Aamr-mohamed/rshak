...
import GenericCalendar from "../../Components/Calender/genericCalender";
...
export default function LastUserInfo() {
  ...
  const handleFoodDateChange = (dates) => {
    setFoodDate(dates);
  };
  const handleAttendaceDateChange = (selectedDates) => {
    setAttendaceDate(selectedDates);
  };

  const userInfoSchema = yup.object().shape({
    // .required("الرجاء إدخال موعد الفحص الشهري.")
    monthlyCheckup: yup.string().required("الرجاء إدخال موعد الفحص الشهري."),
    // monthkyMealsCheckup: yup.string(),
    // // .required("الرجاء إدخال تفاصيل الفحص الشهري للوجبات."),
    // weeklyCheckupTimes: yup
    //   .string()
    //   .required("الرجاء إدخال عدد مرات الفحص الأسبوعي."),
    specialMeals: yup.string().required("الرجاء إدخال تفاصيل الوجبات الخاصة."),
  ...
  const verification = async (values) => {
    values = {
      ...values,
      ...formData,
      selectedAttendaceDates: attendaceDate,
      selectedFoodDates: fooddate,
    };
    console.log(values);
    // try {
    // Make an API call to submit the data
    // await axios.post(`${backendUrl}/adduser`, values);
    // customToast("تم اضافة المستخدم بنجاح", "success");
    // Navigate to the next page after successful submission
    navigate("/LastUserInfo", { state: values });
    // } catch (error) {
              ...
              {(props) => (
                <Form className="flex flex-col w-[100%]">
                  <div className="mb-4 rounded-lg shadow-md">
                    <h1 className="text-center text-[#1769AE] text-xl font-bold mt-5">
                      سجل الحضور الشهري
                    </h1>
                    <GenericCalendar onDateChange={handleAttendaceDateChange} />
                  </div>
                  <div className="mb-4 rounded-lg shadow-md">
                    <h1 className="text-center text-[#1769AE] text-xl font-bold mt-5">
                      سجل صرف الأغذية
                    ...
                    <GenericCalendar onDateChange={handleFoodDateChange} />
                  </div>
